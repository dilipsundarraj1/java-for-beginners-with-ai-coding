...
public class WrapperClassesWithArrayList {
    ...
    /**
     * Calculate letter grade from Integer list of grades
     */
    public static char calculateLetterGrade(List<Integer> grades) {
     ...
     */
    public static TemperatureStats analyzeTemperatures(List<Double> temperatures) {
        ...
        for (Double temp : temperatures) {
            if (temp < min) min = temp;  // Unboxing
            if (temp > max) max = temp;
            sum += temp;
        }

        double average = sum / temperatures.size();
        return new TemperatureStats(min, max, average);
    }

    /**
     * Find missing IDs in sequence
     */
    public static List<Long> findMissingIds(List<Long> existingIds, Long start, Long end) {
        List<Long> missing = new ArrayList<>();

        for (Long id = start; id <= end; id++) {  // Unboxing in loop
            if (!existingIds.contains(id)) {
                missing.add(id);  // Boxing
            }
        }
    ...
    // Helper class for temperature statistics
    static class TemperatureStats {
        private final double min;
        private final double max;
        private final double average;

        public TemperatureStats(double min, double max, double average) {
            this.min = min;
            this.max = max;
        ...
        public String toString() {
            return String.format("Min: %.1f°C, Max: %.1f°C, Average: %.1f°C", min, max, average);
        }
    }
}
