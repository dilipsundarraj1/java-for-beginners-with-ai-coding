...
public class StreamGatherersDemo {
    ...
    private static void demonstrateCompositeGatherers(List<Movie> movies) {
        ...
        // Filter high-rated movies, then group into windows, then calculate average duration per window
        movies.stream()
                ...
                .gather(Gatherers.scan(
                        () -> 0.0,
                        (avgDuration, window) -> {
                            double windowAverage = window.stream()
                ...
                .forEach(avgDuration ->
                        System.out.println("High-rated movie window average duration: " +
                                String.format("%.1f", avgDuration) + " minutes"));
